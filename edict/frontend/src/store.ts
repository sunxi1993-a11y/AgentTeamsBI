...
export const PIPE = [
  ...
  { key: 'Taizi',    dept: '总指挥', icon: '', action: '接收' },
  { key: 'Zhongshu', dept: '总指挥', icon: '', action: '分拣' },
  { key: 'Menxia',   dept: '总指挥', icon: '', action: '规划' },
  { key: 'Assigned', dept: '总指挥', icon: '', action: '派发' },
  { key: 'Doing',    dept: '执行',   icon: '', action: '执行' },
  { key: 'Review',   dept: '总指挥', icon: '', action: '汇报' },
  { key: 'Done',     dept: '汇报',   icon: '', action: '完成' },
] as const;

export const PIPE_STATE_IDX: Record<string, number> = {
  Inbox: 0, 
  Taizi: 1, Zhongshu: 2, Menxia: 2, Assigned: 2,  // 总指挥负责分拣规划审核派发
  Doing: 3, 
  Review: 4, Done: 5, 
  Blocked: 3, Cancelled: 3, Next: 2,
};
...
export const STATE_LABEL: Record<string, string> = {
  Inbox: '收件', Pending: '待处理', 
  Taizi: '总指挥接收', Zhongshu: '总指挥分拣',
  Menxia: '总指挥规划', Assigned: '总指挥派发', 
  Doing: '执行中', Review: '总指挥汇报',
...
export type TabKey =
  | 'edicts' | 'monitor' | 'agents' | 'officials' | 'models'
